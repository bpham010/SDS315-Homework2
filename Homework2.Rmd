...
```{r, echo = FALSE}
ggplot(data = profs) + geom_boxplot(aes(x=eval, y = native)) + labs(x = "Average Evaluation Score")
```
...
The above graphs show average number of bike rentals during the day, but this time separated by whether or not they were rented on working days. A working day is a day that isn't a weekend nor holiday, and it shows the same peak at hours 6 and 18 while the non-working days appear to peak at hour 12, which may just be people biking leisurely. 
